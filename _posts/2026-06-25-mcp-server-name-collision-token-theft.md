...
- **W4 — Token lookup by name only.** `isAccessAllowed()` matches the stored grant by `server.id` string — no URL, publisher, or key binding. This directly contravenes MCP's own authorization spec, which requires tokens to be bound to the server's **canonical URI** via the [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html) `resource` indicator — not its name. *(`authenticationMcpAccessService.ts:61–79`)*
...
- [MCP Authorization specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization) — token audience binding, "clients MUST NOT send tokens to a server other than the one they were issued for"
- [MCP Security Best Practices](https://modelcontextprotocol.io/specification/2025-06-18/basic/security_best_practices) — token passthrough, confused deputy, local vs. remote server risks
